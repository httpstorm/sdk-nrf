...
int shtc3_init(const struct device * dev)
{
	shtc3_dev = dev;
...
static int client_id_get(char * id_buf, size_t len)
{
	...
	int err = nrf_modem_at_cmd(imei_buf, sizeof(imei_buf), "%s", "AT+CGSN");

	if (err)
	{
		printk("Error when trying to do nrf_modem_at_cmd: %d, response: %s", err, imei_buf);
	}
...
static int iccid_get(char * id_buf, size_t len)
{
	...
	if (err)
	{
		printk("Error when trying to do nrf_modem_at_cmd: %d, response: %s", err, iccid_buf);
		return -1;
	}
...
int main(void)
{
	...
	adp_err = adp536x_init(I2C_BUS_2_DEV);
