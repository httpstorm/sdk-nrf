...
static int ascii_to_septet(const char * inbuf, char * outbuf, size_t outbuf_len)
{
	...
	while (len - 1 > i)
	{
		...
		int count = snprintf(outbuf, 3, "%02X", septet | ((inbuf[i + 1] & bitmask) << (7 - bitcount)));
		outbuf[count] = '\0';
		outbuf += 2;
		result += 1;
		i += 1;
...
int send_sms(const char * number, const char * msg, char * cmd_buf, size_t buf_len)
{
	...
	err = nrf_modem_at_cmd(imei_buf, sizeof(imei_buf), "%s", "AT+CMFG=0");
	...
	if (AT_CMD_OK == err)
	{
		...
		err = nrf_modem_at_cmd(imei_buf, sizeof(imei_buf), "%s", cmd_buf);
		//-printk("%i %d buf=%s\n\r", err, at_state, imei_buf);
		printk("%i buf=%s\n\r", err, imei_buf);
