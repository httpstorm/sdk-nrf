...
bool ble_get_mac(char addr[13])
{
	...
	const bt_addr_le_t * mac_addr = bt_conn_get_dst(current_conn);
...
K_EVENT_DEFINE(ble_send_event);
static k_tid_t ble_send_task = NULL;

static void ble_send_handler(void * p1, void * p2, void * p3)
{
	ble_send_task = k_current_get();

	while (1)
	{
		k_event_wait(&ble_send_event, 0x1, false, K_FOREVER);
		k_event_set(&ble_send_event, 0x0);
		bt_send_work_handler(&bt_send_work);
	}
}

// BLE_PRIORITY must be higher than UART_PRIORITY
// a lower number implies higher priority
#define BLE_STACK_SIZE 2048
#define BLE_PRIORITY 4

K_THREAD_DEFINE(ble_send_tid, BLE_STACK_SIZE, ble_send_handler, (void *)0, NULL, NULL, BLE_PRIORITY, 0, 0);
...
static void bt_sent_cb(struct bt_conn *conn)
{
	...
	if (ble_send_task)
	{
		k_event_set(&ble_send_event, 0x1);
	}
	else
	{
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	if (is_uart_data_event(aeh)) {
		...
		{
			// forward from nRF9160 to BLE blocked
			return false;
		}
#endif
