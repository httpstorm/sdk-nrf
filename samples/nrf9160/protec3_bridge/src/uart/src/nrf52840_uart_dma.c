...
#if NRF52840_USE_DMA
extern uart_api * nrf52840_links[NRF52840_UARTE_COUNT];
...
fn_int_handler nrf52840_uart_handlers_dma[] =
{
	...
	nrf52840_uart_handler_raw_dma_2,
	nrf52840_uart_handler_raw_dma_3,
#endif
};
#endif
...
void nrf52840_uart_deinit_dma(uart_api * uart)
{
	...
	reg->ENABLE = UARTE_ENABLE_ENABLE_Disabled;
...
UART_STATUS_E nrf52840_uart_init_dma(uart_api * uart)
{
	...
	const uint32_t count_rx = state->max_rx;
#else
	const uint32_t count_rx = state->size;
#endif
...
{
	// disable interrupts
	// a new DMA request must be queued as soon as possible to prevent dropouts
	const int_state_t int_state = int_disable_nested();
...
void __attribute__((interrupt)) nrf52840_uart_handler_raw_dma_2(void)
{
	if (nrf52840_links[2])
	{
		nrf52840_uart_handler_int_dma(nrf52840_links[2], 0);
	}
}
...
void __attribute__((interrupt)) nrf52840_uart_handler_raw_dma_3(void)
{
	if (nrf52840_links[3])
	{
		nrf52840_uart_handler_int_dma(nrf52840_links[3], 0);
	}
}
#endif
...
int nrf52840_uart_handler_int_dma(uart_api * uart, int int_state)
{
	...
	if (NRF52840_UART_INT_ENABLED_RX(reg))
	{
		...
		while (reg->EVENTS_ENDRX || reg->EVENTS_RXDRDY)
		{
			if (reg->EVENTS_ENDRX)
			{
				// RX complete
			...
			if (uart->job_rx.msg && (state->rx_a != state->rx_b))
			{
