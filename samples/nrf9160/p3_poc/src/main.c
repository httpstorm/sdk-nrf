...
#define I2C_BUS_2_DEV           DEVICE_DT_GET(DT_NODELABEL(i2c2))
...
#define EEPROM0_DEVICE          DEVICE_DT_GET(DT_NODELABEL(eeprom0))
//-#define UART_DEVICE_NAME     DT_LABEL(DT_NODELABEL(uart1))
#define UART_DEVICE             DEVICE_DT_GET(DT_NODELABEL(uart1))
...
#define PIN_STRUCT(alias, type) {                                   \
	...
	.gpio_dev = DEVICE_DT_GET(DT_GPIO_CTLR(DT_ALIAS(alias), gpios)),\
	/*.gpio_pin_name = DT_LABEL(DT_ALIAS(alias)),*/                 \
	.gpio_pin_name = DT_PROP(DT_ALIAS(alias), label),               \
...
/// alias.in  = button gps_enable rtc_int pir_int pmic_int accel_int1
/// alias.out = gps_enable alarm_loud
//-const char * test_gpio_dev_name = DT_GPIO_LABEL(DT_ALIAS(button), gpios); // deprecated
const struct device * gpio_dev = DEVICE_DT_GET(DT_GPIO_CTLR(DT_ALIAS(button), gpios));
const char * test_gpio_pin_name = DT_PROP(DT_ALIAS(button), label);
unsigned int test_gpio_pin = DT_GPIO_PIN(DT_ALIAS(button), gpios);
unsigned int test_gpio_flags = GPIO_INPUT | FLAGS_OR_ZERO(DT_ALIAS(button));
///
...
/*
static int adp536x_reg_write_mask(u8_t reg_addr, u32_t mask, u8_t data)
{
	int err;
	u8_t tmp;
...
static void rsp_send(const char * fmt, ...)
{
	char rsp_buf[128] = { 0 };
	va_list arg_ptr = { 0 };
	...
	const int err = nrf_modem_at_cmd(rsp_buf, sizeof(rsp_buf), "%s", rsp_buf);
	printk("rsp_send: nrf_modem_at_cmd(%s) %i", rsp_buf, err);
}
...
{
	const struct device * gpio_dev = gpio->gpio_dev;
	int ret;
...
static int query_modem(const char * cmd, char * buf, size_t buf_len)
{
	...
	if (ret)
	{
		...
		snprintk(buf, buf_len, "AT cmd: %s  ret: %d", cmd, ret);
...
static int iccid_get(char * id_buf, size_t len)
{
	...
	int err = nrf_modem_at_cmd(iccid_buf, sizeof(iccid_buf), "%s", "AT+CRSM=176,12258,0,0,10");
	//int err = nrf_modem_at_cmd(iccid_buf, sizeof(iccid_buf), "%s", "AT%XICCID");
	...
	if (err)
	{
		printk("Error when trying to do at_cmd_write: %d, response: %s", err, iccid_buf);
		//-printk("Error when trying to do at_cmd_write: %d, at_state: %d", err, at_state);
		return -1;
	}
...
static int read_ficr_word(uint32_t * result, const volatile uint32_t * addr)
{
	...
	uint32_t status = 0;
	enum tfm_platform_err_t ret = tfm_platform_mem_read(result, (uint32_t)addr, sizeof(uint32_t), &status);
...
static int data_publish1(struct mqtt_client * c, enum mqtt_qos qos, u8_t * subtopic, u8_t * data)
{
	u8_t topic[64];
	struct mqtt_publish_param param;
	...
	//printk("to topic: %s len: %u\n", topic, (unsigned int)strlen(data));
...
int cert_provision(void)
{
	...
	err = modem_key_mgmt_exists(sec_tag_list[0], MODEM_KEY_MGMT_CRED_TYPE_CA_CHAIN, &exists);
	...
	if (exists)
	{
		 ...
		 */
		err = modem_key_mgmt_delete(sec_tag_list[0], MODEM_KEY_MGMT_CRED_TYPE_CA_CHAIN);
	...
	err = modem_key_mgmt_exists(sec_tag_list[0], MODEM_KEY_MGMT_CRED_TYPE_PRIVATE_CERT, &exists);
	...
	if (exists)
	{
		 ...
		 * with our security tag and reprovision our certificate.
		 */
		err = modem_key_mgmt_delete(sec_tag_list[0], MODEM_KEY_MGMT_CRED_TYPE_PRIVATE_CERT);
	...
	err = modem_key_mgmt_exists(sec_tag_list[0], MODEM_KEY_MGMT_CRED_TYPE_PUBLIC_CERT, &exists);
	...
	//	 * with our security tag and reprovision our certificate.
	//	 */
	//	err = modem_key_mgmt_delete(sec_tag_list[0], MODEM_KEY_MGMT_CRED_TYPE_PUBLIC_CERT);
	//	if (err)
	//	{
	//		printk("Failed to delete existing certificate, err %d\n", err);
	//	}
...
int cert_provision1(nrf_sec_tag_t sec_tag, enum modem_key_mgmt_cred_type cred_type, const void * buf, size_t len)
{
	...
	err = modem_key_mgmt_exists(sec_tag, cred_type, &exists);
...
void accelerometer_delayed_worker(struct k_work * item)
{
	...
	acceleration_mg[0] = iis2dlpc_from_fs2_lp1_to_mg(data_raw_acceleration[0]);
	acceleration_mg[1] = iis2dlpc_from_fs2_lp1_to_mg(data_raw_acceleration[1]);
	acceleration_mg[2] = iis2dlpc_from_fs2_lp1_to_mg(data_raw_acceleration[2]);
	...
	snprintk(
		...
		acceleration_mg[1],
		acceleration_mg[2],
		all_source.wake_up_src.sleep_state_ia ? 1 : 0
	);
...
int main(void)
{
	...
	if (
		(read_ficr_word(&device_id[0], &NRF_FICR_S->INFO.DEVICEID[0]) == 0) &&
		(read_ficr_word(&device_id[1], &NRF_FICR_S->INFO.DEVICEID[1]) == 0)
		)
	{
