...
int gpio_init(void)
{
	...
	if ((GPIO_COUNT_NAMES != GPIO_COUNT_PINS) || (GPIO_COUNT_NAMES != GPIO_COUNT_FLAGS))
	{
		printk("gpio_init()  gpio_names, gpio_pins, and gpio_flags should have the same length\n");
		return FAIL;
	}

	for (i = 0; i < GPIO_COUNT_NAMES; i++)
	{
		gpio.list[i].dev = device_get_binding(gpio_names[0]);
