...
#if CONFIG_PINCTRL
int nrf52840_uart_pinctrl(struct uart_api * uart)
{
...
int nrf52840_uart_framing_error_check(struct uart_api * uart, uint8_t random_delay)
{
...
int nrf52840_uart_send_byte(struct uart_api * uart, char c)
{
	...
	else
	{
		...
		uart_reg state = uart->reg;
		NRF_UART_Type * reg = state->reg;
		int_state_t int_state = NRF52840_UART_INT_DISABLE(reg);
		...
		if ((state->tx_b - state->tx_a) > state->mask)
		{
			...
			int_state = NRF52840_UART_INT_DISABLE(reg);
		}
...
int nrf52840_uart_recv_byte(struct uart_api * uart, char * c)
{
	...
	uint32_t use_interrupts = uart->config.use_interrupts && !int_unavailable();
	...
	if (use_interrupts)
	{
		...
		if (state->rx_a == state->rx_b)
		{
			...
			else
			{
				...
				if (OK != nrf52840_uart_framing_error_check(uart, (uint8_t)SysTick->VAL))
				{
					return FAIL;
				}
