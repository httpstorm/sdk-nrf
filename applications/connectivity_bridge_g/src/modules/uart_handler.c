...
static const uint32_t baud[] = { 9600, 230400, 460800, 921600 };
...
{
	gpio_init();
	const uint16_t gpio_int = GPIOTE_IRQn;
	NRF_GPIOTE->EVENTS_PORT = 0;
...
static void uart_miniport(void * p1, void * p2, void * p3)
{
	...
	if (!uart)
	{
		LOG_ERR("UART[%u]->uart_open  FAILED", (uint32_t)dev_idx);
		return;
	}

	if (!uart->config.enabled)
	{
		LOG_INF("UART[%u] disabled", (uint32_t)dev_idx);
		return;
	}

	LOG_INF("UART[%u] miniport: starting...", (uint32_t)dev_idx);
	...
	// UART init code
	if (UART_STATUS_OK != uart->init(uart))
	{
		LOG_ERR("UART[%u]->init  FAILED", (uint32_t)dev_idx);
		return;
	}

	LOG_INF("UART[%u] ready %u", (uint32_t)dev_idx, uart->baud_rate);
	...
	while (1)
	{
		...
		do
		{
			if (!slab_rx_buf)
			{
				err = k_mem_slab_alloc(&uart_rx_slab, (void **)&slab_rx_buf, count ? K_MSEC(1) : K_NO_WAIT);
			}

			// avoid blocking receive if we already received data and we are waiting for a slab buffer
			if ((count < UART_BUF_SIZE) && (!count || (state->rx_a != state->rx_b)))
			{
			...
			{
				LOG_ERR("UART[%u] RX  k_mem_slab_alloc FAILED %i", (uint32_t)dev_idx, err);
				count = 0;
			}

			if (uart_tasks[dev_idx] && !subscriber_count[dev_idx])
			{
				LOG_INF("UART[%u] power save", (uint32_t)dev_idx);
				uart->deinit(uart);
		...
		if (err)
		{
			LOG_WRN("UART[%u] err %i  count %u  slab %p", (uint32_t)dev_idx, err, (int32_t)count, slab_rx_buf);
		}
...
static void set_uart_baudrate(uint8_t dev_idx, uint32_t baudrate)
{
	...
	if (uart && baudrate && (uart->baud_rate != baudrate))
	{
		...
		if (UART_STATUS_OK != uart->set_baud_rate(uart, baudrate, 0))
		{
...
static int uart_tx_enqueue(uint8_t * data, size_t data_len, uint8_t dev_idx)
{
	...
	if (uart)
	{
		ssize_t count = uart->send(uart, (char *)data, data_len);

		if ((size_t)count == data_len)
		{
			return OK;
		}
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_uart_data_event(aeh)) {
		...
		uart_tx_enqueue(event->buf, event->len, event->dev_idx ? 0 : 1);
#endif
